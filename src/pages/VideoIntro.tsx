...
import { decodeEventData } from '../utils/urlHelper';
import Logo from '../components/Logo';

const THEMES: Record<string, { accent: string, border: string, bgAlpha: string, fill: string }> = {
    classic: {
        accent: 'text-premium-gold',
        border: 'border-premium-gold',
        bgAlpha: 'bg-premium-gold/10',
        fill: 'fill-premium-gold/10'
    },
    romantic: {
        accent: 'text-[#FFB7C5]',
        border: 'border-[#FFB7C5]',
        bgAlpha: 'bg-[#FFB7C5]/10',
        fill: 'fill-[#FFB7C5]/10'
    },
    modern: {
        accent: 'text-[#2D3436]',
        border: 'border-[#2D3436]',
        bgAlpha: 'bg-[#2D3436]/10',
        fill: 'fill-[#2D3436]/10'
    },
};
        ...
        <div className="relative h-screen w-full bg-[#0a0a0a] overflow-hidden flex items-center justify-center font-sans">
                ...
                {showOverlay && (
                    ...
                    >
                        <motion.div
                            ...
                            className="mb-12"
                        >
                            <div className={`w-20 h-20 border ${theme.border}/30 rounded-3xl flex items-center justify-center mx-auto mb-6 bg-white/5 backdrop-blur-xl shadow-[0_0_50px_rgba(212,175,55,0.15)] relative`}>
                                <div className={`absolute inset-0 rounded-3xl border ${theme.border}/10 animate-ping opacity-20`} />
                                <Logo showText={false} className="scale-125" dark />
                            </div>
                        </motion.div>
                        ...
                        >
                            A Special <br />
                            <span className={theme.accent}>Invitation</span>
                        </motion.h1>
                        ...
                        <motion.button
                            ...
                            onClick={handleStart}
                            className={`group relative px-16 py-6 border ${theme.border}/50 text-white rounded-full font-bold overflow-hidden transition-all hover:bg-neutral-100 hover:text-black hover:border-white active:scale-95 shadow-[0_0_30px_rgba(212,175,55,0.15)]`}
                        >
            ...
            {hasStarted && (
                ...
                >
                    ...
                    <button
                        ...
                        className={`p-4 ${theme.bgAlpha} backdrop-blur-xl rounded-full text-white border ${theme.border}/30 hover:bg-white transition-colors hover:text-black group`}
                    >
            ...
            {hasStarted && videoRef.current && (
                <div className="absolute bottom-0 left-0 w-full h-1 bg-white/10 z-50">
                    <motion.div
                        className={`h-full ${theme.accent.replace('text-', 'bg-')} shadow-[0_0_10px_rgba(212,175,55,1)]`.replace('bg-[#FFB7C5]', 'bg-[#FFB7C5] shadow-[0_0_10px_rgba(255,183,197,1)]').replace('bg-[#2D3436]', 'bg-[#2D3436] shadow-[0_0_10px_rgba(45,52,54,1)]')}
                        animate={{ width: "100%" }}
